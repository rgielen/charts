...
4. renders the pages at <https://rgielen.github.io/charts/> from the index it just wrote.

## The published site

The pages next to `index.yaml` are generated by `.github/scripts/build_pages.py` from that
index and the chart directories. Nothing on `gh-pages` is written by hand, and a chart page
needs no work of its own: it is the chart's generated README plus the versions the index
says are published.

Two things do affect it, and both live here:

- the chart's `values.yaml` comments and `README.md.gotmpl`, which is where every word on a
  chart page comes from;
...
The index has to come from the branch: it is the record of what has been published, and no
part of it can be derived from a working tree.
